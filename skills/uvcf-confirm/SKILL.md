...
Write `.vcf/<slug>/REVIEW-slice-N.md`: PASS or the defect list, the round count, and any deferred lower-severity items (so the next slice's BUILD reads them). Log decisions to LEDGER.md. **On PASS, advance `STATUS.md`** — mark slice N done and set `Current phase:` to the next slice's `build` (or `verify` if this was the last slice). In gate mode the next `/ultimate-vcf:uvcf-build` reads STATUS.md to know which slice is next, so this write-back is mandatory. Then hand off: next slice's `/ultimate-vcf:uvcf-build`, or `/ultimate-vcf:uvcf-verify` if this was the last slice.
